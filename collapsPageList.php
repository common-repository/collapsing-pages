<?php
/*
Collapsing Pages version: 2.0.3
...
function getSubPage($page, $pages, $parents,$subPageCount, $curDepth, $expanded) {
    ...
    foreach ($pages as $page2) {
      ...
      if ($page->ID==$page2->post_parent) {
         ...
         * */
          ...
          if ( in_array( $page2->post_name, $autoExpand ) ||
              in_array( $page2->post_title, $autoExpand ) ||
          ...
          list ($subPageLink2, $subPageCount)= getSubPage($page2, $pages, $parents,$subPageCount, $curDepth,$expanded);
  ...
  return array($subPageLinks,$subPageCount);
}
...
function list_pages($args) {
  ...
  if ($debug==1) {
    $collapse_page_text .= "<li style='display:none' >";
	$collapse_page_text .= print_r($args, true);
    $collapse_page_text .= "\ncollapsPage options:\n";
    $collapse_page_text .= print_r($options, true);
    $collapse_page_text .= "PAGE QUERY: \n $pagequery\n";
    $collapse_page_text .= "\nPAGE QUERY RESULTS\n";
    $collapse_page_text .= print_r($pages, true);
    $collapse_page_text .= "\nAUTOEXPAND\n";
    $collapse_page_text .= print_r($autoExpand, true);
    $collapse_page_text .= "</li>";
  }
    ...
    if ($page->post_parent==0) {
      ...
      if ( in_array($page->post_name, $autoExpand ) ||
          in_array( $page->ID, $autoExpand ) ||
              ( $useCookies && isset( $_COOKIE[ $theID ] ) && 1 === $_COOKIE[ $theID ] ) ) {
        $expanded='block';
      }
      $curDepth=0;
      if ($depth!=0) {
        list ( $subPageLinks, $subPageCount ) =
